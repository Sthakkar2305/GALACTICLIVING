    ...
    <article className="group overflow-hidden rounded-[28px] border border-black/8 bg-white shadow-[0_18px_60px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-luxury">
      <div className="relative aspect-[4/3] overflow-hidden">
        <Image
          ...
          alt={property.name}
          fill
          className={`object-cover transition duration-700 group-hover:scale-105 ${isComingSoon ? "blur-sm grayscale" : ""}`}
          sizes="(min-width: 1024px) 33vw, 100vw"
        />
        <div className="absolute left-4 top-4 rounded-full bg-white/92 px-3 py-1 text-xs font-bold text-galactic-red shadow">
          {property.badge ?? "Preferred by students"}
        </div>
        {!isComingSoon && (
        ...
        {isComingSoon && (
          <div className="absolute inset-0 grid place-items-center bg-black/42 p-6 text-center text-white backdrop-blur-[2px]">
            <div>
              <p className="text-3xl font-black">Coming Soon</p>
              <p className="mt-2 text-sm text-white/78">New Galactic residence launching shortly.</p>
            </div>
          </div>
        )}
      </div>
      <div className="p-5">
        ...
        <div className="mt-5 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-2xl font-black">Rs. {property.price.toLocaleString("en-IN")}<span className="text-sm font-semibold text-black/46"> /mo</span></p>
          </div>
          {isComingSoon ? (
            <span className="rounded-full bg-black/8 px-4 py-3 text-sm font-bold text-black/45">Soon</span>
          ) : (
